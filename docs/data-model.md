...
CRM частного массажного салона. СУБД — PostgreSQL 16, доступ через Prisma 6.x.
...
2. Миграция создаётся скриптом `pnpm db:migration <name>`: он выполняет `prisma migrate dev --create-only` и дописывает соответствующий SQL-файл в конец `migration.sql`. Ручного копирования нет.
...
**Версия Prisma зафиксирована на 6.x** (`prisma` и `@prisma/client` без диапазона `^`). В Prisma 7 строка подключения переезжает в `prisma.config.ts`, и смешение инструкций двух мажоров сломало бы обещание «`docker compose up` и всё работает» для нового разработчика. Переход на 7 — отдельная задача с обновлением README.
